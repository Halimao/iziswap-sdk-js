...
async function main(): Promise<void> {
    ...
    // 代理服务器的URL
    const proxy = 'http://127.0.0.1:40880';

    // 创建代理代理器
    const agent = new HttpsProxyAgent(proxy);

    const web3 = new Web3(new Web3.providers.HttpProvider(rpc, { timeout: 10000, agent: { https: agent } }))
    ...
    console.log('preQueryResult: ', preQueryResult)
